...
#    return ' \\\\\\\\ '.join(f"\\vspace*{{\\fill}}${step[0]} \\times {step[1]} = \\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_$\\vspace*{{\\fill}}" for step in unique_steps)

    # Return the unique steps as a string with the last step in a larger font
    return ' \\\\ '.join(f"\\vspace*{{\\fill}}${step[0]} \\times {step[1]} = \\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_$\\vspace*{{\\fill}}" if step != unique_steps[-1] else f"\\vspace*{{\\fill}}\\textbf{{\\LARGE ${step[0]} \\times {step[1]} = \\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_\\_$}}\\vspace*{{\\fill}}" for step in unique_steps)
...
def generate_latex_table():
    ...
    latex_code += "\\hline\n"
    for _ in range(3):
        latex_code += f"\\begin{{minipage}}[c][0.33\\textheight][c]{{0.5\\textwidth}}\n{create_multiplication_steps()}\\\\\n\\end{{minipage}} & "
        latex_code += f"\\begin{{minipage}}[c][0.33\\textheight][c]{{0.5\\textwidth}}\n{create_multiplication_steps()}\\\\\n\\end{{minipage}} \\\\\n\\hline\n"
